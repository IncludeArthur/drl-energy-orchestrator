...
#use CPU consumption instead of power consumption for reward
class NfvAllocCPUEnv(gym.Env):

    ''' #### ENVIRONMENT FUNCTIONS #### '''

    def __init__(self, config_file, dt_scale, duration_mean, duration_scale, qi_probabilities=None):
        ...
        self.cloud_node = CloudHost(power_coef=host_config['cloud']['power_coef'], latency=host_config['cloud']['latency'], cpu_coef=0.01)

        edge_hosts_config = host_config['edge']
        self.n_edges = len(edge_hosts_config)
        self.edge_nodes = []
        for node in edge_hosts_config:
            self.edge_nodes.append(EdgeHost(power_model=node['power_model'], 
        ...
        self.qi_probabilities = qi_probabilities
        ...
        self.reward_latency_normalization = 0.01
    ...
    def step(self, action):
        ...
        if latency_error > 0 : 
            reward -= latency_error * self.reward_latency_normalization
    ...
    def _get_obs(self):
        ...
        cloud_observations = np.array([self.cloud_node.get_traffic(), 
                                       0.0, 
                                       self.cloud_node.get_cpu_consumption(), 
    ...
    def get_new_session(self):
        ...
        #qos = self.rng.integers(0,3) # 0,1,2 these are the 5QI (QoS Indicators) not standard
        qos = self.rng.choice([0,1,2], p = qi_probabilities)
        duration = self.rng.normal(loc=self.duration_mean, scale=self.duration_scale)
        #duration = self.duration_mean
        throughput = self.rng.uniform(low=10, high=100)
        #throughput = 50
        
        session = PDUSession(qos, duration, throughput, self.time)
